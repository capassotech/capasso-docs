import type { Project, EnvVar, BranchConvention, CommitStep, PlatformGroup } from '../types'

// ── Convenciones compartidas ───────────────────────────────────────────────────

const BRANCH_CONVENTIONS: BranchConvention[] = [
...
const BACKEND_FIREBASE_PRIVATE_KEY_QA =
...
const BACKEND_FIREBASE_PRIVATE_KEY_PROD =
...
const BACKEND_ENV_QA: EnvVar[] = [
  ...
  { key: 'FIREBASE_PRIVATE_KEY', value: BACKEND_FIREBASE_PRIVATE_KEY_QA, desc: 'Clave privada del service account QA (escapar saltos de línea)' },
...
const BACKEND_ENV_PROD: EnvVar[] = [
  ...
  { key: 'FIREBASE_PRIVATE_KEY', value: BACKEND_FIREBASE_PRIVATE_KEY_PROD, desc: 'Clave privada del service account prod' },
...
function frontendEnv(opts: { apiUrl: string } & typeof QA_FIREBASE): EnvVar[] {
  return [
...
const SHARED_PLATFORMS: PlatformGroup[] = [
  {
    category: 'Gestión del proyecto',
    links: [
      { label: 'Jira — gestión integral', url: 'https://capassotech.atlassian.net/jira/software/projects/EPEFI/boards/170', desc: 'Gestión integral, tableros, sprints, cronogramas y más' },
      { label: 'Trello — tablero EPEFI', url: 'https://trello.com/b/btjMNcoi/epefi', desc: 'Tickets y seguimiento de tareas del equipo' },
    ],
  },
]

const BACKEND_PLATFORMS: PlatformGroup[] = [
  { category: 'Repositorio', links: [{ label: 'epefi-backend', url: 'https://github.com/capassotech/epefi-backend', desc: 'Código fuente del API REST en GitHub' }] },
  { category: 'Deploy', links: [{ label: 'Render Dashboard', url: 'https://dashboard.render.com/', desc: 'Web service del backend (QA y producción)' }] },
  {
    category: 'Firebase (Admin SDK)',
    links: [
      { label: 'Firebase — QA (epefi-admin-qa)', url: 'https://console.firebase.google.com/u/0/project/epefi-admin-qa/overview', desc: 'Firestore, Auth y Storage usados por el backend en QA' },
      { label: 'Firebase — Prod (epefi-admin)', url: 'https://console.firebase.google.com/u/0/project/epefi-admin/overview', desc: 'Firestore, Auth y Storage usados por el backend en producción' },
    ],
  },
]

const CURSOS_PLATFORMS: PlatformGroup[] = [
  { category: 'Repositorio', links: [{ label: 'epefi-cursos', url: 'https://github.com/capassotech/epefi-cursos', desc: 'Código fuente del portal del alumno en GitHub' }] },
  {
    category: 'Firebase (Hosting)',
    links: [
      { label: 'Firebase — QA (epefi-cursos-qa)', url: 'https://console.firebase.google.com/u/0/project/epefi-cursos-qa/overview', desc: 'Proyecto Firebase de cursos en QA' },
      { label: 'Firebase — Prod (epefi-cursos)', url: 'https://console.firebase.google.com/u/0/project/epefi-cursos/overview', desc: 'Proyecto Firebase de cursos en producción' },
    ],
  },
]

const ADMIN_PLATFORMS: PlatformGroup[] = [
  { category: 'Repositorio', links: [{ label: 'epefi-admin', url: 'https://github.com/capassotech/epefi-admin', desc: 'Código fuente del panel de administración en GitHub' }] },
  {
    category: 'Firebase (DB, Auth, Storage y Hosting)',
    links: [
      { label: 'Firebase — QA (epefi-admin-qa)', url: 'https://console.firebase.google.com/u/0/project/epefi-admin-qa/overview', desc: 'Proyecto Firebase del admin en QA' },
      { label: 'Firebase — Prod (epefi-admin)', url: 'https://console.firebase.google.com/u/0/project/epefi-admin/overview', desc: 'Proyecto Firebase del admin en producción' },
    ],
  },
]
...
export const epefi: Project = {
  id: 'epefi',
  name: 'EPEFI',
  color: '#F97316',
  description: 'Plataforma educativa para la EPEFI. Gestiona cursos, exámenes y progreso de alumnos.',
  ...
  subprojects: [
    // ── Backend ──────────────────────────────────────────────────────────────
    {
      ...
      commitFlow: COMMIT_FLOW,
      envGroups: [
        { name: 'local', label: 'Local (DEV)', vars: BACKEND_ENV_QA },
        { name: 'qa', label: 'QA', vars: BACKEND_ENV_QA },
        { name: 'prod', label: 'Producción', vars: BACKEND_ENV_PROD },
      ],
      deploys: [
        {
    ...
    // ── Cursos ───────────────────────────────────────────────────────────────
    {
      ...
      commitFlow: COMMIT_FLOW,
      envGroups: [
        { name: 'local', label: 'Local (DEV)', vars: frontendEnv({ apiUrl: 'http://localhost:3000', ...QA_FIREBASE }) },
        { name: 'qa', label: 'QA', vars: frontendEnv({ apiUrl: 'https://epefi-backend-qa.onrender.com', ...QA_FIREBASE }) },
        { name: 'prod', label: 'Producción', vars: frontendEnv({ apiUrl: 'https://epefi-backend.onrender.com', ...PROD_FIREBASE }) },
      ],
      deploys: [
        {
